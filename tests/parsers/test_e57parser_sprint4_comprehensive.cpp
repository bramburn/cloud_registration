...
protected:
    ...
    void setupTestEnvironment()
    {
        ...
        availableTestFiles = {"sample/bunnyDouble.e57", "sample/bunnyInt32.e57"};
...
TEST_F(E57Sprint4ComprehensiveTest, ComprehensiveFunctionalityVerification)
{
    ...
    if (totalValidTests > 0)
    {
        ...
        qDebug() << "Comprehensive test results:" << passedTests << "/" << totalValidTests << "passed (" << successRate
                 << "%)";
    }
...
TEST_F(E57Sprint4ComprehensiveTest, PerformanceProfilingAndOptimization)
{
    ...
    if (baselineMetrics.success)
    {
        ...
        qDebug() << "Baseline performance:" << baselineMetrics.pointsPerSecond << "points/sec,"
        ...
        profiler->generatePerformanceReport(optimizationResults, testDataDir + "/performance_report.html");
    }
}
...
TEST_F(E57Sprint4ComprehensiveTest, PerformanceKPIValidation)
{
    ...
    if (metrics.success && metrics.pointCount > 0)
    {
        ...
        if (pointsInMillions > 0.001)
        {  // Only test for files with significant point count
            EXPECT_GE(metrics.pointsPerSecond, MIN_POINTS_PER_SECOND) << "Performance below minimum threshold";
...
TEST_F(E57Sprint4ComprehensiveTest, MultiScanHandling)
{
    ...
    {
        ASSERT_TRUE(parser->openFile(testFile.toStdString())) << "Failed to open test file: " << testFile;
        ...
        if (scanCount > 0)
        {
            ...
            EXPECT_EQ(directPointCount, scanMetadata.pointCount) << "Point count mismatch between methods";
...
TEST_F(E57Sprint4ComprehensiveTest, EnhancedPointDataExtraction)
{
    ...
    if (parser->getScanCount() > 0)
    {
        ...
        if (!enhancedPoints.empty())
        {
            ...
            bool hasAnyIntensity =
                std::any_of(enhancedPoints.begin(), enhancedPoints.end(), [](const auto& p) { return p.hasIntensity; });
            bool hasAnyColor =
                std::any_of(enhancedPoints.begin(), enhancedPoints.end(), [](const auto& p) { return p.hasColor; });
