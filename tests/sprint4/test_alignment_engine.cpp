...
void TestAlignmentEngine::testCorrespondenceManagement()
{
    ...
    // Test initial state
    QVERIFY2(m_engine->getCorrespondences().isEmpty(), "Engine should start with no correspondences");
    ...
    QVERIFY2(m_engine->getCorrespondences().size() == 1, "Should have one correspondence after adding");
    QVERIFY2(correspondencesSpy.count() == 1, "Should emit correspondencesChanged signal");
    ...
    QVERIFY2(m_engine->getCorrespondences().isEmpty(), "Should have no correspondences after clearing");
}
...
void TestAlignmentEngine::testQualityThresholds()
{
    ...
    AlignmentEngine::AlignmentResult result = m_engine->getCurrentResult();
    QVERIFY2(result.errorStats.rmsError > 0.5f, "Should have measurable RMS error with misaligned correspondences");
...
void TestAlignmentEngine::testStateTransitions()
{
    ...
    AlignmentEngine::AlignmentResult result = m_engine->getCurrentResult();
    QVERIFY2(result.state == AlignmentEngine::AlignmentState::Idle, "Should start in Idle state");
...
void TestAlignmentEngine::testAutoRecompute()
{
    ...
    AlignmentEngine::AlignmentResult result = m_engine->getCurrentResult();
    QVERIFY2(result.state != AlignmentEngine::AlignmentState::Valid, "Should not auto-compute when disabled");
    ...
    result = m_engine->getCurrentResult();
    QVERIFY2(result.state == AlignmentEngine::AlignmentState::Valid, "Manual computation should work");
...
void TestAlignmentEngine::testAutoRecomputeConfiguration()
{
    ...
    QVERIFY2(m_engine->isAutoRecompute() == !initialState, "Auto-recompute setting should be toggled");

    // Restore original state
    m_engine->setAutoRecompute(initialState);
    QVERIFY2(m_engine->isAutoRecompute() == initialState, "Auto-recompute setting should be restored");
}
...
void TestAlignmentEngine::testInsufficientCorrespondences()
{
    ...
    QVERIFY2(result.transformation.isIdentity(), "Transformation should be identity for insufficient correspondences");
}
...
void TestAlignmentEngine::testEmptyEngine()
{
    ...
    AlignmentEngine::AlignmentResult result = m_engine->getCurrentResult();
    QVERIFY2(result.state == AlignmentEngine::AlignmentState::Idle, "Empty engine should be in Idle state");
    QVERIFY2(result.transformation.isIdentity(), "Empty engine should have identity transformation");
    QVERIFY2(result.errorStats.numCorrespondences == 0, "Empty engine should have zero correspondences in stats");
}
