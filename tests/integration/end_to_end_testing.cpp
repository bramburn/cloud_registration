...
protected:
    ...
    {
        ON_CALL(*mockView, getPointCloudViewer()).WillByDefault(Return(mockViewer.get()));

        ON_CALL(*mockView, showStatusMessage(_)).WillByDefault(Return());

        ON_CALL(*mockView, updateProgressBar(_, _)).WillByDefault(Return());

        ON_CALL(*mockViewer, loadPointCloud(_)).WillByDefault(Return(true));

        ON_CALL(*mockViewer, clearPointCloud()).WillByDefault(Return());
    }
...
TEST_F(FullRegistrationWorkflowTest, CompleteRegistrationWorkflow)
{
    ...
    EXPECT_CALL(*mockView, showStatusMessage(testing::HasSubstr("project"))).Times(AtLeast(1));

    // Step 2: Import first scan file
    EXPECT_CALL(*mockViewer, loadPointCloud(_)).Times(AtLeast(1)).WillRepeatedly(Return(true));
    ...
    ASSERT_LT(refinedRmsError, initialRmsError) << "ICP should improve alignment accuracy";
