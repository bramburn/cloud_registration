...
TEST_F(IntegrationTest, FullWorkflowProjectCreationToFileLoading) {
    ...
    
    {
        ...
        EXPECT_CALL(*m_parser, extractPointData())
            .Times(1)
        ...
        EXPECT_CALL(*m_view, updateStatusBar(testing::HasSubstr("loaded successfully")))
            .Times(1);
    }
...
TEST_F(IntegrationTest, ErrorHandlingWorkflow) {
    ...
    EXPECT_CALL(*m_view, displayErrorMessage(
        testing::HasSubstr("File Opening"),
        testing::HasSubstr("File not found")
    )).Times(1);
    
    EXPECT_CALL(*m_view, updateStatusBar(testing::HasSubstr("Failed")))
        .Times(1);
