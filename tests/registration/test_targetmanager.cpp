...
protected:
    void SetUp() override
    {
        ...
        naturalTarget1 =
            std::make_shared<NaturalPointTarget>("natural_001", QVector3D(7.0f, 8.0f, 9.0f), "Corner point");
...
TEST_F(TargetManagerTest, AddCorrespondences)
{
    ...
    TargetCorrespondence correspondence(sphereTarget1->targetId(), sphereTarget2->targetId(), scanId1, scanId2);
...
TEST_F(TargetManagerTest, CorrespondenceValidation)
{
    ...
    TargetCorrespondence invalidCorr(sphereTarget1->targetId(), "non_existent_target", scanId1, scanId2);

    EXPECT_FALSE(targetManager->addCorrespondence(invalidCorr));

    // Try to add correspondence between targets in same scan
    targetManager->addTarget(scanId1, sphereTarget2);
    TargetCorrespondence sameScanCorr(sphereTarget1->targetId(),
...
TEST_F(TargetManagerTest, CalculateStatistics)
{
    ...
    TargetCorrespondence correspondence(sphereTarget1->targetId(), naturalTarget1->targetId(), scanId1, scanId2);
    targetManager->addCorrespondence(correspondence);
