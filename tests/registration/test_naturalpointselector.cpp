...
TEST_F(NaturalPointSelectorTest, SelectPointFromScreen) {
    ...
    auto result = selector->selectPoint(points, viewMatrix, projectionMatrix, 
                                      screenPos, viewportSize, 10.0f);
...
TEST_F(NaturalPointSelectorTest, SuggestCorrespondences) {
    ...
    auto correspondences = selector->suggestCorrespondences(
        sourcePoints, targetPoints, sourceSelection, 0.2f);
...
TEST_F(NaturalPointSelectorTest, FeatureAnalysis) {
    ...
    if (planarResult.success) {
        ...
        EXPECT_TRUE(planarResult.description.contains("surface") || 
                   planarResult.description.contains("Planar"));
    }
    ...
    if (edgeResult.success) {
        ...
        EXPECT_TRUE(edgeResult.description.contains("edge") || 
                   edgeResult.description.contains("Linear") ||
    ...
    if (cornerResult.success) {
        ...
        EXPECT_TRUE(cornerResult.description.contains("corner") || 
                   cornerResult.description.contains("Point") ||
...
TEST_F(NaturalPointSelectorTest, EmptyPointCloud) {
    ...
    auto result = selector->selectPoint(emptyPoints, viewMatrix, projectionMatrix, 
...
TEST_F(NaturalPointSelectorTest, RayCalculations) {
    ...
    auto result = selector->selectPoint(points, viewMatrix, projectionMatrix, 
                                      centerScreen, viewportSize, 50.0f);
...
TEST_F(NaturalPointSelectorTest, FeatureSimilarity) {
    ...
    if (result1.success && result2.success) {
        ...
        auto correspondences = selector->suggestCorrespondences(
            points, points, result1, 2.0f);  // Large search radius
