...
public:
    ...
    MOCK_METHOD(bool,
                setFileMetadata,
                (const QString& guid, const QString& description, const QString& creationDateTime),
                (override));
    ...
    void setupSuccessfulWriting(const QString& filePath = "test_output.e57")
    {
        ...
        ON_CALL(*this, addScan(_)).WillByDefault(Return(true));

        ON_CALL(*this, writePoints(_, _)).WillByDefault(Return(true));

        ON_CALL(*this, writeScanData(_)).WillByDefault(Return(true));

        ON_CALL(*this, closeFile()).WillByDefault(Return(true));

        ON_CALL(*this, getCurrentFilePath()).WillByDefault(Return(filePath));
    ...
    void setupFailedWriting(const QString& errorMessage = "Mock writing error")
    {
        ...
        ON_CALL(*this, createFile(_)).WillByDefault(Return(false));

        ON_CALL(*this, isOpen()).WillByDefault(Return(false));

        ON_CALL(*this, addScan(_)).WillByDefault(Return(false));

        ON_CALL(*this, writePoints(_, _)).WillByDefault(Return(false));

        ON_CALL(*this, writeScanData(_)).WillByDefault(Return(false));

        ON_CALL(*this, closeFile()).WillByDefault(Return(false));

        ON_CALL(*this, getCurrentFilePath()).WillByDefault(Return(QString()));
    ...
    void setupPartialFailure(bool createSuccess = true, bool addScanSuccess = true, bool writePointsSuccess = false)
    {
        using ::testing::_;
        using ::testing::Return;

        ON_CALL(*this, createFile(_)).WillByDefault(Return(createSuccess));

        ON_CALL(*this, addScan(_)).WillByDefault(Return(addScanSuccess));

        ON_CALL(*this, writePoints(_, _)).WillByDefault(Return(writePointsSuccess));

        ON_CALL(*this, isOpen()).WillByDefault(Return(createSuccess));

        if (!writePointsSuccess)
        {
            ON_CALL(*this, getLastError()).WillByDefault(Return("Failed to write points"));
        }
    }
    ...
    static std::vector<Point3D> createTestPoints(int numPoints = 10)
    {
        ...
        {
            points.emplace_back(static_cast<double>(i), static_cast<double>(i + 1), static_cast<double>(i + 2));
        }
    ...
    createTestExportOptions(bool includeIntensity = false, bool includeColor = false, bool compressData = true)
    {
    ...
    static ScanData createTestScanData(int numPoints = 10, bool includeIntensity = false, bool includeColor = false)
    {
        ScanData scanData;
