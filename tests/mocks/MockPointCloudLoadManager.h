...
public:
    ...
    {
        ON_CALL(*this, loadScan(testing::_)).WillByDefault(testing::Return(true));
        ON_CALL(*this, unloadScan(testing::_)).WillByDefault(testing::Return(true));
        ON_CALL(*this, isLoading()).WillByDefault(testing::Return(false));
    }

    void setupFailedLoading()
    {
        ON_CALL(*this, loadScan(testing::_)).WillByDefault(testing::Return(false));
        ON_CALL(*this, unloadScan(testing::_)).WillByDefault(testing::Return(false));
    }

    void setupScanLoadedState(const QString& scanId, bool isLoaded)
    {
        ON_CALL(*this, isScanLoaded(scanId)).WillByDefault(testing::Return(isLoaded));
    }

    void setupLoadingState(bool isLoading)
    {
        ON_CALL(*this, isLoading()).WillByDefault(testing::Return(isLoading));
    }
