...
public:
    explicit MockMainView(QObject *parent = nullptr) 
        : IMainView(parent)
        , m_mockViewer(std::make_unique<MockPointCloudViewer>()) {}
    ...
    void setupSuccessfulFileDialog(const QString& filePath) {
        ...
        ON_CALL(*this, askForOpenFilePath(_, _))
            .WillByDefault(Return(filePath));
        
        ON_CALL(*this, askForSaveFilePath(_, _, _))
            .WillByDefault(Return(filePath));
    }
    ...
    void setupConfirmationDialog(bool userConfirms) {
        ...
        ON_CALL(*this, askForConfirmation(_, _))
            .WillByDefault(Return(userConfirms));
    }
    ...
    void verifyErrorDisplayed(const QString& expectedTitle = QString(), 
                            const QString& expectedMessage = QString()) {
        using ::testing::_;
        
        if (expectedTitle.isEmpty() && expectedMessage.isEmpty()) {
            EXPECT_CALL(*this, displayErrorMessage(_, _))
                .Times(::testing::AtLeast(1));
        } else if (expectedTitle.isEmpty()) {
            EXPECT_CALL(*this, displayErrorMessage(_, expectedMessage))
                .Times(1);
        } else if (expectedMessage.isEmpty()) {
            EXPECT_CALL(*this, displayErrorMessage(expectedTitle, _))
                .Times(1);
        } else {
            EXPECT_CALL(*this, displayErrorMessage(expectedTitle, expectedMessage))
                .Times(1);
        }
    }

    void verifyInfoDisplayed(const QString& expectedTitle = QString(), 
                           const QString& expectedMessage = QString()) {
        using ::testing::_;
        
        if (expectedTitle.isEmpty() && expectedMessage.isEmpty()) {
            EXPECT_CALL(*this, displayInfoMessage(_, _))
                .Times(::testing::AtLeast(1));
        } else {
            EXPECT_CALL(*this, displayInfoMessage(expectedTitle, expectedMessage))
                .Times(1);
        }
    }
    ...
    void verifyStatusUpdated(const QString& expectedText = QString()) {
        ...
        if (expectedText.isEmpty()) {
            EXPECT_CALL(*this, updateStatusBar(_))
                .Times(::testing::AtLeast(1));
        } else {
            EXPECT_CALL(*this, updateStatusBar(expectedText))
                .Times(1);
        }
    }
    ...
    void verifyWindowTitleSet(const QString& expectedTitle = QString()) {
        ...
        if (expectedTitle.isEmpty()) {
            EXPECT_CALL(*this, setWindowTitle(_))
                .Times(::testing::AtLeast(1));
        } else {
            EXPECT_CALL(*this, setWindowTitle(expectedTitle))
                .Times(1);
        }
    }
    ...
    void verifyProgressDialogShown(bool shouldShow = true) {
        ...
        EXPECT_CALL(*this, showProgressDialog(shouldShow, _, _))
            .Times(::testing::AtLeast(1));
    }
    ...
    void verifyProgressUpdated() {
        ...
        EXPECT_CALL(*this, updateProgress(_, _))
            .Times(::testing::AtLeast(1));
    }

    void verifyActionsEnabled(bool enabled) {
        EXPECT_CALL(*this, setActionsEnabled(enabled))
            .Times(::testing::AtLeast(1));
    }
    ...
    void verifyScanListUpdated(const QStringList& expectedScans = QStringList()) {
        ...
        if (expectedScans.isEmpty()) {
            EXPECT_CALL(*this, updateScanList(_))
                .Times(::testing::AtLeast(1));
        } else {
            EXPECT_CALL(*this, updateScanList(expectedScans))
                .Times(1);
        }
    }

    void verifyProjectViewShown() {
        EXPECT_CALL(*this, showProjectView())
            .Times(1);
    }

    void verifyProjectHubShown() {
        EXPECT_CALL(*this, showProjectHub())
            .Times(1);
    }

    void verifyFileDialogCalled(const QString& expectedTitle = QString(),
                              const QString& expectedFilter = QString()) {
        using ::testing::_;
        
        if (expectedTitle.isEmpty() && expectedFilter.isEmpty()) {
            EXPECT_CALL(*this, askForOpenFilePath(_, _))
                .Times(::testing::AtLeast(1));
        } else {
            EXPECT_CALL(*this, askForOpenFilePath(expectedTitle, expectedFilter))
                .Times(1);
        }
    }

    void verifyConfirmationAsked(const QString& expectedTitle = QString(),
                               const QString& expectedMessage = QString()) {
        using ::testing::_;
        
        if (expectedTitle.isEmpty() && expectedMessage.isEmpty()) {
            EXPECT_CALL(*this, askForConfirmation(_, _))
                .Times(::testing::AtLeast(1));
        } else {
            EXPECT_CALL(*this, askForConfirmation(expectedTitle, expectedMessage))
                .Times(1);
        }
    }
