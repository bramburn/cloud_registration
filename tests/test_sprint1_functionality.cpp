...
TEST_F(Sprint1Test, LasHeaderMetadataStructure) {
    ...
    metadata.minBounds = {-10.0, -5.0, 0.0};
    metadata.maxBounds = {10.0, 5.0, 20.0};

    EXPECT_EQ(metadata.numberOfPointRecords, 12345);
    EXPECT_EQ(metadata.filePath, "test.las");
    EXPECT_DOUBLE_EQ(metadata.minBounds.x, -10.0);
    EXPECT_DOUBLE_EQ(metadata.minBounds.y, -5.0);
    EXPECT_DOUBLE_EQ(metadata.minBounds.z, 0.0);
    EXPECT_DOUBLE_EQ(metadata.maxBounds.x, 10.0);
    EXPECT_DOUBLE_EQ(metadata.maxBounds.y, 5.0);
    EXPECT_DOUBLE_EQ(metadata.maxBounds.z, 20.0);
}
