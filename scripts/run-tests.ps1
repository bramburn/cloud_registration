...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "Usage: .\scripts\run-tests.ps1 [options]" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host "  .\scripts\run-tests.ps1 -Coverage          # Run tests with coverage"
    Write-Host "  .\scripts\run-tests.ps1 -Filter '*E57*'    # Run only E57 tests"
    Write-Host "  .\scripts\run-tests.ps1 -Verbose           # Show detailed output"
}
...
try {
    ...
    if (-not $gtestFound) {
        ...
        Write-Host "  - Windows: vcpkg install gtest" -ForegroundColor White
    ...
    Write-Host "✓ All test executables built successfully" -ForegroundColor Green
    ...
    $script:finalExitCode = $testExitCode
