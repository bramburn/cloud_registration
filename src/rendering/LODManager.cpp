...
void LODManager::configureLOD(float nearDistance, float farDistance, int maxPointsPerFrame)
{
    ...
    qDebug() << "LODManager configured - Near:" << m_nearDistance 
             << "Far:" << m_farDistance 
...
std::vector<float> LODManager::getVisiblePointData(float aspectRatio, const std::vector<float>& pointData)
{
    ...
    float reductionRatio = m_lastTotalPoints > 0 ? 
        static_cast<float>(m_lastVisiblePoints) / m_lastTotalPoints : 0.0f;
...
bool LODManager::isNodeInFrustum(const QVector3D& nodeCenter, float nodeSize, 
                                const QMatrix4x4& viewProjectionMatrix) const
{
    ...
    for (const QVector4D& plane : planes) {
        ...
        for (int i = 0; i < 8; ++i) {
            float distance = plane.x() * corners[i].x() + 
                           plane.y() * corners[i].y() + 
                           plane.z() * corners[i].z() + 
                           plane.w();
            if (distance >= 0) {
...
void LODManager::extractFrustumPlanes(const QMatrix4x4& viewProjectionMatrix, 
                                     std::vector<QVector4D>& planes) const
{
    ...
    for (QVector4D& plane : planes) {
        float length = std::sqrt(plane.x() * plane.x() + 
                               plane.y() * plane.y() + 
                               plane.z() * plane.z());
        if (length > 0.0f) {
            plane /= length;
...
bool LODManager::isPointInFrustum(const QVector3D& point, 
