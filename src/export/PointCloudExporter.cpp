...
#include "FormatWriters/XYZWriter.h"
#include <QDebug>
#include <QFileInfo>
...
ExportResult PointCloudExporter::exportPointCloud(const std::vector<Point>& points, const ExportOptions& options)
{
    ...
    try {
        ...
        qDebug() << "PointCloudExporter: Successfully exported" << pointsWritten 
                 << "points to" << options.outputPath 
...
{
    switch (format) {
        case ExportFormat::E57: return ".e57";
        case ExportFormat::LAS: return ".las";
        case ExportFormat::PLY: return ".ply";
        case ExportFormat::XYZ: return ".xyz";
        default: return ".dat";
    }
}
...
std::vector<Point> PointCloudExporter::transformCoordinates(const std::vector<Point>& points,
