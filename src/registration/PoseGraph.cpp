...
bool PoseGraph::removeNode(int nodeIndex) {
    ...
    auto nodeIt = std::find_if(m_nodes.begin(), m_nodes.end(),
        [nodeIndex](const PoseNode& node) { return node.nodeIndex == nodeIndex; });
...
int PoseGraph::findNodeByScanId(const QString& scanId) const {
    auto it = std::find_if(m_nodes.begin(), m_nodes.end(),
        [&scanId](const PoseNode& node) { return node.scanId == scanId; });
...
bool PoseGraph::isValidNodeIndex(int index) const {
    return std::any_of(m_nodes.begin(), m_nodes.end(),
        [index](const PoseNode& node) { return node.nodeIndex == index; });
}
