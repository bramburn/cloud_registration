...
TargetDetectionBase::TargetDetectionBase(QObject* parent) : QObject(parent) {}

bool TargetDetectionBase::validateParameters(const DetectionParams& params) const
{
    if (params.distanceThreshold <= 0.0f || params.distanceThreshold > 1.0f)
        return false;
    if (params.maxIterations <= 0 || params.maxIterations > 10000)
        return false;
    if (params.minQuality < 0.0f || params.minQuality > 1.0f)
        return false;
    if (params.minRadius <= 0.0f || params.maxRadius <= params.minRadius)
        return false;
    if (params.minInliers <= 0)
        return false;
    if (params.neighborhoodRadius <= 0.0f)
        return false;
    if (params.curvatureThreshold < 0.0f)
        return false;
...
std::vector<PointFullData> TargetDetectionBase::preprocessPoints(const std::vector<PointFullData>& points,
...
void TargetDetectionBase::removeOutliers(std::vector<PointFullData>& points, int meanK, float stddevMulThresh) const
{
    if (points.size() < static_cast<size_t>(meanK))
        return;
...
{
    if (voxelSize <= 0.0f || points.empty())
        return;
...
{
    return QString("%1_%2_%3").arg(prefix).arg(QDateTime::currentMSecsSinceEpoch()).arg(++s_targetIdCounter);
}
...
{
    if (map.contains("distanceThreshold"))
        distanceThreshold = map["distanceThreshold"].toFloat();
    if (map.contains("maxIterations"))
        maxIterations = map["maxIterations"].toInt();
    if (map.contains("minQuality"))
        minQuality = map["minQuality"].toFloat();
    if (map.contains("enablePreprocessing"))
        enablePreprocessing = map["enablePreprocessing"].toBool();
    if (map.contains("minRadius"))
        minRadius = map["minRadius"].toFloat();
    if (map.contains("maxRadius"))
        maxRadius = map["maxRadius"].toFloat();
    if (map.contains("minInliers"))
        minInliers = map["minInliers"].toInt();
    if (map.contains("neighborhoodRadius"))
        neighborhoodRadius = map["neighborhoodRadius"].toFloat();
    if (map.contains("curvatureThreshold"))
        curvatureThreshold = map["curvatureThreshold"].toFloat();
}
