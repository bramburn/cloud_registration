...
float ErrorAnalysis::calculateRMSError(const QList<QPair<QVector3D, QVector3D>>& correspondences,
...
ErrorAnalysis::calculateErrorStatistics(const QList<QPair<QVector3D, QVector3D>>& correspondences,
                                        const QMatrix4x4& transform)
{
...
QList<float> ErrorAnalysis::calculateIndividualErrors(const QList<QPair<QVector3D, QVector3D>>& correspondences,
                                                      const QMatrix4x4& transform)
{
    QList<float> errors;
...
QList<int> ErrorAnalysis::identifyOutliers(const QList<QPair<QVector3D, QVector3D>>& correspondences,
                                           const QMatrix4x4& transform,
                                           float outlierThreshold)
{
    ...
    for (int i = 0; i < errors.size(); ++i)
    {
        if (errors[i] > threshold)
        {
            ...
            qDebug() << "Outlier detected at index" << i << "with error" << errors[i] << "mm (threshold:" << threshold
                     << "mm)";
        }
    }

    qDebug() << "Identified" << outlierIndices.size() << "outliers out of" << correspondences.size()
             << "correspondences";
    return outlierIndices;
}
...
float ErrorAnalysis::calculateConditionNumber(const QList<QPair<QVector3D, QVector3D>>& correspondences)
{
