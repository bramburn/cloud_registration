                                                            ...
                                                            const DetectionParams& params)
{
    ...
    {
        result.errorMessage =
            QString("Insufficient points for sphere detection. Need at least %1 points.").arg(MIN_POINTS_FOR_SPHERE);
        return result;
    }
    ...
    for (int sphereIndex = 0; sphereIndex < MAX_SPHERES_PER_CLOUD; ++sphereIndex)
    {
        ...
        std::vector<int> inliers = findInliers(processedPoints, sphere, params.distanceThreshold, usedPoints);
        for (int idx : inliers)
        {
            usedPoints[idx] = true;
        }

        qDebug() << "Detected sphere" << sphereIndex + 1 << "- Center:" << sphere.center << "Radius:" << sphere.radius
    ...
    qDebug() << "SphereDetector: Detected" << result.targets.size() << "spheres in" << result.processingTime
             << "seconds";
                                                               ...
                                                               const std::vector<bool>& usedPoints) const
{
    ...
    for (int iteration = 0; iteration < params.maxIterations; ++iteration)
    {
        ...
        // Find inliers
        std::vector<int> inliers = findInliers(points, candidateSphere, params.distanceThreshold, usedPoints);

        if (static_cast<int>(inliers.size()) > bestInlierCount && static_cast<int>(inliers.size()) >= params.minInliers)
        {
...
SphereDetector::SphereModel SphereDetector::fitSphereToPoints(const QVector3D& p1,
                                                              const QVector3D& p2,
                                                              const QVector3D& p3,
                                                              const QVector3D& p4) const
{
    SphereModel sphere;
    ...
    float maxDeviation = std::max(
        {std::abs(r1 - avgRadius), std::abs(r2 - avgRadius), std::abs(r3 - avgRadius), std::abs(r4 - avgRadius)});

    if (maxDeviation < avgRadius * 0.2f)
    {  // Allow 20% deviation
...
SphereDetector::SphereModel SphereDetector::refineSphereModel(const std::vector<PointFullData>& points,
...
float SphereDetector::calculateQuality(const SphereModel& sphere, int totalPoints, const DetectionParams& params) const
{
...
bool SphereDetector::validateSphere(const SphereModel& sphere, const DetectionParams& params) const
{
    if (!sphere.isValid())
    {
        return false;
...
std::vector<SphereDetector::SphereModel>
SphereDetector::removeOverlappingSpheres(const std::vector<SphereModel>& spheres, float overlapThreshold) const
{
    if (spheres.size() <= 1)
    {
        return spheres;
    ...
    std::sort(indices.begin(),
              indices.end(),
              [&spheres](size_t a, size_t b) { return spheres[a].quality > spheres[b].quality; });
...
std::vector<int> SphereDetector::generateRandomSample(const std::vector<PointFullData>& points,
