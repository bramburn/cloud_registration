...
FeatureBasedRegistration::Result FeatureBasedRegistration::registerPointClouds(const std::vector<Point3D>& sourcePoints,
                                                                               ...
                                                                               const Parameters& params)
{
    ...
    try
    {
        ...
        QList<Features::Plane> sourcePlanes = m_featureExtractor->extractPlanes(sourcePoints, params.extractionParams);
        ...
        QList<Features::Plane> targetPlanes = m_featureExtractor->extractPlanes(targetPoints, params.extractionParams);
        ...
        if (sourcePlanes.size() < params.minCorrespondences || targetPlanes.size() < params.minCorrespondences)
        {
            result.errorMessage = QString("Insufficient planes found (source: %1, target: %2, min: %3)")
                                      .arg(sourcePlanes.size())
                                      .arg(targetPlanes.size())
                                      .arg(params.minCorrespondences);
        ...
        if (result.correspondences.size() < params.minCorrespondences)
        {
            result.errorMessage = QString("Insufficient correspondences found: %1 (min: %2)")
                                      .arg(result.correspondences.size())
                                      .arg(params.minCorrespondences);
            qWarning() << result.errorMessage;
            return result;
        }
        ...
        if (params.validateResult)
        {
            ...
            if (result.quality < params.minRegistrationQuality)
            {
                result.errorMessage = QString("Registration quality too low: %1 (min: %2)")
                                          .arg(result.quality)
                                          .arg(params.minRegistrationQuality);
                qWarning() << result.errorMessage;
                return result;
            }
        }
        ...
        qDebug() << "Feature-based registration completed successfully in" << timer.elapsed() << "ms";
        qDebug() << "Quality:" << result.quality << "Correspondences:" << result.correspondencesFound;
...
QList<QPair<Features::Plane, Features::Plane>> FeatureBasedRegistration::findPlaneCorrespondences(
    const QList<Features::Plane>& sourcePlanes, const QList<Features::Plane>& targetPlanes, const Parameters& params)
{
    ...
    for (int i = 0; i < sourcePlanes.size(); ++i)
    {
        ...
        if (bestTargetIndex >= 0 && bestSimilarity > 0.5f)
        {
            ...
            qDebug() << "Found correspondence" << i << "->" << bestTargetIndex << "similarity:" << bestSimilarity;
        }
    }
...
float FeatureBasedRegistration::validateRegistrationQuality(const Result& result,
...
FeatureBasedRegistration::Parameters
FeatureBasedRegistration::getRecommendedParameters(const std::vector<Point3D>& sourcePoints,
                                                   const std::vector<Point3D>& targetPoints) const
{
    Parameters params;
...
float FeatureBasedRegistration::calculatePlaneSimilarity(const Features::Plane& plane1,
...
QMatrix4x4 FeatureBasedRegistration::solveForRotation(const QList<QVector3D>& sourceNormals,
...
QVector3D FeatureBasedRegistration::solveForTranslation(const QList<QVector3D>& sourceCentroids,
...
QMatrix3x3 FeatureBasedRegistration::computeCrossCovariance(const QList<QVector3D>& sourceVectors,
...
float FeatureBasedRegistration::calculateRegistrationError(
    const QList<QPair<Features::Plane, Features::Plane>>& correspondences, const QMatrix4x4& transformation) const
{
