    ...
    try {
        ...
        for (const auto& reg : registrations) {
            ...
            if (sourceNode >= 0 && targetNode >= 0) {
                ...
                qWarning() << "Could not find nodes for registration:" 
                          << reg.sourceScanId << "->" << reg.targetScanId;
            }
        ...
        qDebug() << "Built pose graph with" << graph->nodeCount() << "nodes and" 
...
PoseGraphBuilder::ValidationResult PoseGraphBuilder::validateGraph(const PoseGraph& graph) const {
    ...
    result.isValid = (result.connectedComponents <= 1 || 
                     (result.connectedComponents == result.isolatedScans.size() + 1));
    ...
    qDebug() << "Graph validation:" << result.isValid 
             << "Components:" << result.connectedComponents
             << "Loops:" << result.hasLoops
             << "Isolated:" << result.isolatedScans.size();
...
void PoseGraphBuilder::dfsVisit(const PoseGraph& graph, int startNode, 
