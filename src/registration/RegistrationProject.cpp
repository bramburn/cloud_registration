...
{
    connect(targetManager_.get(), &TargetManager::dataChanged,
            this, &RegistrationProject::markAsModified);
    connect(targetManager_.get(), &TargetManager::targetAdded,
            this, &RegistrationProject::markAsModified);
    connect(targetManager_.get(), &TargetManager::targetRemoved,
            this, &RegistrationProject::markAsModified);
}
...
void RegistrationProject::addRegistrationResult(const RegistrationResult& result)
{
    ...
    qDebug() << "RegistrationProject: Added registration result for" 
             << result.sourceScanId << "to" << result.targetScanId;
}

void RegistrationProject::removeRegistrationResult(const QString& sourceScanId, const QString& targetScanId)
{
    auto it = std::find_if(registrationResults_.begin(), registrationResults_.end(),
    ...
    if (it != registrationResults_.end()) {
        ...
        markAsModified();
        qDebug() << "RegistrationProject: Removed registration result for" 
                 << sourceScanId << "to" << targetScanId;
    }
}
...
RegistrationProject::RegistrationResult RegistrationProject::getRegistrationResult(const QString& sourceScanId, const QString& targetScanId) const
{
    auto it = std::find_if(registrationResults_.begin(), registrationResults_.end(),
        [&](const RegistrationResult& result) {
            return (result.sourceScanId == sourceScanId && result.targetScanId == targetScanId) ||
...
{
    return std::any_of(registrationResults_.begin(), registrationResults_.end(),
        [&](const RegistrationResult& result) {
            return (result.sourceScanId == sourceScanId && result.targetScanId == targetScanId) ||
                   (result.sourceScanId == targetScanId && result.targetScanId == sourceScanId);
        });
...
void RegistrationProject::setRegistrationState(RegistrationState state)
{
    if (registrationState_ != state) {
        ...
        qDebug() << "RegistrationProject: Registration state changed to" 
                 << registrationStateToString(state);
    }
}
