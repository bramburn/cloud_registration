...
public:
    ...
    struct Ray
    {
        ...
        Ray() = default;
        Ray(const QVector3D& orig, const QVector3D& dir) : origin(orig), direction(dir.normalized()) {}
    };
...
public:
     ...
     */
    std::vector<SelectionResult> suggestCorrespondences(const std::vector<PointFullData>& sourcePoints,
...
private:
     ...
     */
    int findClosestPointToRay(const std::vector<PointFullData>& points, const Ray& ray, float maxDistance = 0.1f) const;
     ...
     */
    QVector3D analyzeLocalFeatures(const std::vector<PointFullData>& points, int pointIndex, float radius = 0.1f) const;
     ...
     */
    float calculateFeatureSimilarity(const QVector3D& feature1, const QVector3D& feature2) const;
     ...
     */
    std::vector<int> findNeighbors(const std::vector<PointFullData>& points, int centerIndex, float radius) const;
