...
struct PoseNode
{
    ...
    PoseNode(const QString& id, const QMatrix4x4& t, int idx = -1) : scanId(id), transform(t), nodeIndex(idx) {}
};
...
struct PoseEdge
{
    ...
    PoseEdge(int from, int to, const QMatrix4x4& transform, float info = 1.0f, float rms = 0.0f)
        : fromNodeIndex(from), toNodeIndex(to), relativeTransform(transform), informationMatrix(info), rmsError(rms)
    {
    }
};
...
public:
    ...
    const QList<PoseNode>& nodes() const
    {
        return m_nodes;
    }
    const QList<PoseEdge>& edges() const
    {
        return m_edges;
    }
    QList<PoseNode>& nodes()
    {
        return m_nodes;
    }
    QList<PoseEdge>& edges()
    {
        return m_edges;
    }
