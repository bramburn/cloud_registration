...
void ICPProgressWidget::startMonitoring(ICPRegistration* icpAlgorithm, int maxIterations) {
    ...
    connect(m_icpAlgorithm, &ICPRegistration::progressUpdated,
            this, &ICPProgressWidget::updateProgress);
    connect(m_icpAlgorithm, &ICPRegistration::computationFinished,
            this, &ICPProgressWidget::onComputationFinished);
    
    // Connect cancel signal
    connect(this, &ICPProgressWidget::cancelRequested,
            m_icpAlgorithm, &ICPRegistration::cancel);
...
void ICPProgressWidget::updateProgress(int iteration, float rmsError, const QMatrix4x4& /*transformation*/) {
    ...
    // Update UI
    int progressPercent = (m_maxIterations > 0) ? 
        (iteration * 100) / m_maxIterations : 0;
...
void ICPProgressWidget::onComputationFinished(bool success, const QMatrix4x4& /*finalTransformation*/,
    ...
    QString message;
    if (success) {
        float improvement = (m_initialError > 0) ? 
            ((m_initialError - finalRMSError) / m_initialError) * 100.0f : 0.0f;
