...
void MemoryManager::PointDataPool::growPool()
{
    ...
    qDebug() << "MemoryManager: Pool grown to" << m_chunks.size() << "chunks," << m_totalAllocated << "bytes allocated";
}
...
PointFullData* MemoryManager::allocatePoint()
{
    ...
    m_stats.hitRatio = static_cast<double>(m_stats.poolHits) / (m_stats.poolHits + m_stats.poolMisses);
...
bool MemoryManager::initializeStreaming(size_t totalPoints, size_t chunkSize)
{
    ...
    qDebug() << "MemoryManager: Streaming initialized for" << totalPoints << "points in" << numChunks << "chunks";
