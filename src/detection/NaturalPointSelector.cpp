...
TargetDetectionBase::DetectionResult NaturalPointSelector::detect(
...
NaturalPointSelector::SelectionResult NaturalPointSelector::selectPoint(
    const std::vector<PointFullData>& points,
    ...
    float selectionRadius)
{
    ...
    qDebug() << "Selected natural point at" << result.selectedPoint 
             << "with confidence" << result.confidence;
    
    return result;
}

NaturalPointSelector::SelectionResult NaturalPointSelector::selectClosestPoint(
    const std::vector<PointFullData>& points,
...
std::vector<NaturalPointSelector::SelectionResult> NaturalPointSelector::suggestCorrespondences(
    ...
    float searchRadius)
{
    ...
    for (size_t i = 0; i < targetPoints.size(); ++i) {
        ...
        if (distance <= searchRadius) {
            ...
            QVector3D candidateFeatures = analyzeLocalFeatures(targetPoints, static_cast<int>(i), m_defaultSearchRadius);
    ...
    for (int i = 0; i < maxSuggestions; ++i) {
        ...
        suggestion.selectedPoint = QVector3D(targetPoints[candidateIndex].x, 
                                           targetPoints[candidateIndex].y, 
                                           targetPoints[candidateIndex].z);
...
void NaturalPointSelector::onMouseClick(const std::vector<PointFullData>& points,
                                       ...
                                       const QSize& viewportSize)
{
    SelectionResult result = selectPoint(points, viewMatrix, projectionMatrix, 
                                       screenPos, viewportSize, m_defaultSelectionRadius);
...
NaturalPointSelector::Ray NaturalPointSelector::createRayFromScreen(
    const QPoint& screenPos,
    const QSize& viewportSize,
    const QMatrix4x4& viewMatrix,
...
QVector3D NaturalPointSelector::analyzeLocalFeatures(const std::vector<PointFullData>& points,
                                                    int pointIndex,
                                                    float radius) const
{
    ...
    cxx /= count; cyy /= count; czz /= count;
    cxy /= count; cxz /= count; cyz /= count;
...
float NaturalPointSelector::calculateFeatureSimilarity(const QVector3D& feature1,
...
std::vector<int> NaturalPointSelector::findNeighbors(const std::vector<PointFullData>& points,
                                                    int centerIndex,
                                                    float radius) const
{
