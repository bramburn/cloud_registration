...
QVector<float> DifferenceAnalysis::calculateDistances(const std::vector<Point3D>& sourcePoints,
                                                      ...
                                                      const Parameters& params)
{
    ...
    qDebug() << "Calculating distances between" << sourcePoints.size() << "source and" << targetPoints.size()
             << "target points";
    ...
    qDebug() << "Distance calculation completed:" << distances.size() << "distances calculated in" << timer.elapsed()
             << "ms";
...
DifferenceAnalysis::Statistics DifferenceAnalysis::calculateStatistics(const QVector<float>& distances,
                                                                       const Parameters& params) const
{
    ...
    stats.outlierPercentage =
        (stats.totalPoints > 0) ? (static_cast<float>(stats.outliers) / stats.totalPoints) * 100.0f : 0.0f;
...
QVector<float> DifferenceAnalysis::generateColorMapValues(const QVector<float>& distances, float maxDistance) const
{
...
DifferenceAnalysis::Parameters
DifferenceAnalysis::getRecommendedParameters(const std::vector<Point3D>& sourcePoints,
...
QString DifferenceAnalysis::generateAnalysisReport(const Statistics& statistics, const Parameters& params) const
{
    QString report;
    ...
    report += QString("Outliers: %1 (%2%)\n\n").arg(statistics.outliers).arg(statistics.outlierPercentage, 0, 'f', 1);
    ...
    report +=
        QString("Registration Quality Score: %1/1.0 (%2%)\n").arg(quality, 0, 'f', 3).arg(quality * 100.0f, 0, 'f', 1);
...
std::unique_ptr<DifferenceAnalysis::KDTree::Node> DifferenceAnalysis::KDTree::buildTree(std::vector<Point3D>& points,
                                                                                        int depth) const
{
    if (points.empty())
    {
...
float DifferenceAnalysis::KDTree::searchNearest(const Node* node,
                                                const Point3D& query,
                                                float maxDistance,
                                                int depth) const
{
    ...
    switch (axis)
    {
        case 0:
            queryValue = query.x;
            nodeValue = node->point.x;
            break;
        case 1:
            queryValue = query.y;
            nodeValue = node->point.y;
            break;
        case 2:
            queryValue = query.z;
            nodeValue = node->point.z;
            break;
        default:
            queryValue = nodeValue = 0.0f;
            break;
    }
...
// Helper methods
float DifferenceAnalysis::bruteForceNearestDistance(const Point3D& query,
                                                    const std::vector<Point3D>& targetPoints,
