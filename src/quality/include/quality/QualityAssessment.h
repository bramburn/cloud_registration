...
struct QualityPoint {
    ...
    QualityPoint(float x_, float y_, float z_, float intensity_) 
...
public:
    ...
    // Configuration
    void setToleranceThreshold(float tolerance) { m_toleranceThreshold = tolerance; }
    void setDensityRadius(float radius) { m_densityRadius = radius; }
    void setMinCorrespondences(size_t count) { m_minCorrespondences = count; }
...
private:
    ...
    float calculateLocalDensity(const std::vector<QualityPoint>& cloud, 
