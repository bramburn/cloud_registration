...
struct Plane {
     ...
     */
    bool isSimilarTo(const Plane& other, float angleThreshold = 0.087f, // ~5 degrees
...
private:
     ...
     */
    std::pair<QVector3D, float> fitPlaneToThreePoints(const QVector3D& p1,
                                                      const QVector3D& p2,
     ...
     */
    Plane refinePlane(const std::vector<Point3D>& points,
