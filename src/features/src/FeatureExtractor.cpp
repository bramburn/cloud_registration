...
QList<Plane> FeatureExtractor::extractPlanes(const std::vector<Point3D>& points, 
    ...
    qDebug() << "Plane extraction completed:" << extractedPlanes.size() 
             << "planes found in" << timer.elapsed() << "ms";
...
Plane FeatureExtractor::extractSinglePlane(const std::vector<Point3D>& points,
                                          const PlaneExtractionParams& params) {
    return ransacPlaneFitting(points, params);
}

FeatureExtractor::PlaneExtractionParams FeatureExtractor::getRecommendedParameters(
...
float FeatureExtractor::validatePlaneQuality(const Plane& plane, 
                                            const std::vector<Point3D>& points) const {
...
Plane FeatureExtractor::ransacPlaneFitting(const std::vector<Point3D>& points,
    ...
    for (int iter = 0; iter < params.maxIterations; ++iter) {
        ...
        // Count inliers
        QList<int> inliers = countInliers(points, normal, distance, 
                                         params.distanceThreshold, usedIndices);
...
std::pair<QVector3D, float> FeatureExtractor::fitPlaneToThreePoints(const QVector3D& p1,
                                                                   const QVector3D& p2,
...
Plane FeatureExtractor::refinePlane(const std::vector<Point3D>& points,
    ...
    QVector3D candidates[] = {
        QVector3D(1, 0, 0), QVector3D(0, 1, 0), QVector3D(0, 0, 1),
        QVector3D(1, 1, 0).normalized(), QVector3D(1, 0, 1).normalized(), QVector3D(0, 1, 1).normalized()
    };
...
std::pair<QVector3D, float> FeatureExtractor::calculatePlaneProperties(
    const std::vector<Point3D>& points, const QList<int>& inlierIndices) const {
...
bool FeatureExtractor::passesNormalFilter(const Plane& plane,
                                         const PlaneExtractionParams& params) const {
