...
BundleAdjustment::optimize(const Registration::PoseGraph& initialGraph, 
    ...
    try {
        ...
        for (int iteration = 0; iteration < params.maxIterations; ++iteration) {
            ...
            if (params.verbose) {
                qDebug() << "Iteration" << iteration << "Error:" << currentError 
                         << "Lambda:" << lambda << "Delta norm:" << 
        ...
        result.improvementRatio = (result.initialError > 0) ? 
            (result.initialError - result.finalError) / result.initialError : 0.0;
        result.optimizationTimeSeconds = timer.elapsed() / 1000.0;
        
        qDebug() << "Bundle adjustment completed:" << result.statusMessage
...
BundleAdjustment::Parameters BundleAdjustment::getRecommendedParameters(
...
double BundleAdjustment::calculateTotalError(const StateVector& state, 
                                           const Registration::PoseGraph& graph) const {
...
std::vector<std::vector<double>> BundleAdjustment::computeJacobian(
    const StateVector& state, const Registration::PoseGraph& graph) const {
...
std::vector<double> BundleAdjustment::solveLinearSystem(
...
std::unique_ptr<Registration::PoseGraph> BundleAdjustment::createOptimizedGraph(
    const Registration::PoseGraph& originalGraph, const StateVector& optimizedState) const {
...
std::vector<double> BundleAdjustment::Pose6DOF::toVector() const {
    return {
        static_cast<double>(translation.x()), static_cast<double>(translation.y()), static_cast<double>(translation.z()),
        static_cast<double>(rotation.x()), static_cast<double>(rotation.y()), static_cast<double>(rotation.z())
    };
}
...
QMatrix4x4 BundleAdjustment::StateVector::getPoseMatrix(int nodeIndex) const {
    ...
    for (int i = 0; i < m_poses.size(); ++i) {
        if ((m_fixFirstPose && i == 0 && nodeIndex == 0) ||
            (!m_fixFirstPose && i == nodeIndex) ||
