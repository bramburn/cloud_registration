...
struct CRSPoint {
    ...
    CRSPoint(double x_, double y_, double z_, const QString& crs) 
        : x(x_), y(y_), z(z_), sourceCRS(crs) {}
    
    QVector3D toVector3D() const { return QVector3D(x, y, z); }
    void fromVector3D(const QVector3D& vec) { x = vec.x(); y = vec.y(); z = vec.z(); }
};
...
public:
    ...
    std::vector<CRSPoint> transformPoints(const std::vector<CRSPoint>& points, 
                                         const QString& targetCRS);
    
    // Transformation parameters
    TransformationParameters getTransformationParameters(const QString& sourceCRS, 
    ...
    // Configuration
    void setDefaultCRS(const QString& code) { m_defaultCRS = code; }
    QString getDefaultCRS() const { return m_defaultCRS; }
...
private:
    ...
    double degToRad(double degrees) const { return degrees * M_PI / 180.0; }
    double radToDeg(double radians) const { return radians * 180.0 / M_PI; }
