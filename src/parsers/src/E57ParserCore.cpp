...
E57ParserCore::E57ParserCore() : m_imageFile(nullptr) {}
...
std::pair<int, int> E57ParserCore::getVersion() const
{
    ...
    try
    {
        // Note: majorVersion/minorVersion methods may not be available in newer E57Format versions
        // Return default version for now
        return {1, 0};
    }
    catch (const e57::E57Exception&)
    {
...
bool E57ParserCore::inspectPointPrototype(const e57::StructureNode& scanHeader)
{
    try
    {
        ...
        if (!prototype.isDefined("cartesianX") || !prototype.isDefined("cartesianY") ||
        ...
        if (prototype.isDefined("intensity"))
        {
            ...
            m_prototypeInfo.intensityDataType = (intensityNode.type() == e57::TypeFloat) ? "float" : "integer";
        }
        ...
        if (m_prototypeInfo.hasColorRed || m_prototypeInfo.hasColorGreen || m_prototypeInfo.hasColorBlue)
        {
            ...
            if (prototype.isDefined("colorRed"))
            {
                ...
                m_prototypeInfo.colorDataType = (colorNode.type() == e57::TypeFloat) ? "float" : "integer";
            }
        }
    ...
    try
    {
            ...
            settings.loadColor)
        {
            if (m_prototypeInfo.hasColorRed)
                redData.resize(static_cast<size_t>(pointsToRead));
            if (m_prototypeInfo.hasColorGreen)
                greenData.resize(static_cast<size_t>(pointsToRead));
            if (m_prototypeInfo.hasColorBlue)
                blueData.resize(static_cast<size_t>(pointsToRead));
        }

        // Create CompressedVectorReader with required buffers
        std::vector<e57::SourceDestBuffer> buffers;

        // Add XYZ buffers
        buffers.emplace_back(*m_imageFile, "cartesianX", xData.data(), static_cast<size_t>(pointsToRead), true, true);
        buffers.emplace_back(*m_imageFile, "cartesianY", yData.data(), static_cast<size_t>(pointsToRead), true, true);
        buffers.emplace_back(*m_imageFile, "cartesianZ", zData.data(), static_cast<size_t>(pointsToRead), true, true);

        // Add intensity buffer if needed
        if (!intensityData.empty())
        {
            buffers.emplace_back(
                *m_imageFile, "intensity", intensityData.data(), static_cast<size_t>(pointsToRead), true, true);
        }

        // Add color buffers if needed
        if (!redData.empty())
        {
            buffers.emplace_back(
                *m_imageFile, "colorRed", redData.data(), static_cast<size_t>(pointsToRead), true, true);
        }
        if (!greenData.empty())
        {
            buffers.emplace_back(
                *m_imageFile, "colorGreen", greenData.data(), static_cast<size_t>(pointsToRead), true, true);
        }
        if (!blueData.empty())
        {
            buffers.emplace_back(
                *m_imageFile, "colorBlue", blueData.data(), static_cast<size_t>(pointsToRead), true, true);
        }

        e57::CompressedVectorReader reader = cvNode.reader(buffers);
        ...
        for (uint64_t i = 0; i < pointsRead; ++i)
        {
            ...
            {
                point.red =
                    !redData.empty() ? static_cast<uint8_t>(std::min(255.0, static_cast<double>(redData[i]))) : 0;
                point.green =
                    !greenData.empty() ? static_cast<uint8_t>(std::min(255.0, static_cast<double>(greenData[i]))) : 0;
                point.blue =
                    !blueData.empty() ? static_cast<uint8_t>(std::min(255.0, static_cast<double>(blueData[i]))) : 0;
                point.hasColor = true;
            }
...
bool E57ParserCore::validatePointData(const CorePointData& point, const CoreLoadingSettings& settings) const
{
    ...
    if (settings.enableSpatialFilter)
    {
        if (point.x < settings.filterMinX || point.x > settings.filterMaxX || point.y < settings.filterMinY ||
            point.y > settings.filterMaxY || point.z < settings.filterMinZ || point.z > settings.filterMaxZ)
        {
            return false;
        }
    }
...
void E57ParserCore::applyVoxelFilter(std::vector<CorePointData>& points, double voxelSize) const
{
    ...
    for (const auto& point : points)
    {
        ...
        // Create voxel key
        std::string voxelKey = std::to_string(voxelX) + "," + std::to_string(voxelY) + "," + std::to_string(voxelZ);
