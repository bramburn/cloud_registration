...
std::vector<float> LasParser::parse(const QString& filePath, const LoadingSettings& settings)
{
    ...
    try
    {
        ...
        qDebug() << "Header bounding box: Min(" << header.minX << "," << header.minY << "," << header.minZ << ") Max("
                 << header.maxX << "," << header.maxY << "," << header.maxZ << ")";
        qDebug() << "Scale factors: X=" << header.xScaleFactor << " Y=" << header.yScaleFactor
                 << " Z=" << header.zScaleFactor;
        ...
        else if (settings.method == LoadingMethod::VoxelGrid)
        {
            ...
            emit parsingFinished(true,
                                 QString("Successfully loaded %1 points (filtered from %2)")
                                     .arg(points.size() / 3)
                                     .arg(header.numberOfPointRecords),
                                 points);
        }
        else
        {
        ...
        if (!points.empty() && points.size() >= 9)
        {
            ...
            if (points.size() >= 6)
            {
                ...
                {
                    qDebug() << "Sample coordinates - Middle point:" << points[midIndex] << points[midIndex + 1]
                             << points[midIndex + 2];
                }
            }
            size_t lastIndex = points.size() - 3;
            qDebug() << "Sample coordinates - Last point:" << points[lastIndex] << points[lastIndex + 1]
                     << points[lastIndex + 2];
        }
...
bool LasParser::readHeader(QFile& file, LasHeader& header)
{
    ...
    if (!readValue(file, header.fileSourceId) || !readValue(file, header.globalEncoding))
    {
        return false;
    }
    ...
    if (!readValue(file, header.versionMajor) || !readValue(file, header.versionMinor))
    {
        return false;
    }
    ...
    if (!readValue(file, header.creationDayOfYear) || !readValue(file, header.creationYear) ||
        !readValue(file, header.headerSize) || !readValue(file, header.pointDataOffset) ||
        !readValue(file, header.numberOfVLRs) || !readValue(file, header.pointDataFormat) ||
        !readValue(file, header.pointDataRecordLength) || !readValue(file, header.numberOfPointRecords))
    {
        return false;
    }
    ...
    if (!readValue(file, header.xScaleFactor) || !readValue(file, header.yScaleFactor) ||
        !readValue(file, header.zScaleFactor) || !readValue(file, header.xOffset) || !readValue(file, header.yOffset) ||
        !readValue(file, header.zOffset) || !readValue(file, header.maxX) || !readValue(file, header.minX) ||
        !readValue(file, header.maxY) || !readValue(file, header.minY) || !readValue(file, header.maxZ) ||
...
bool LasParser::validateHeader(const LasHeader& header)
{
    ...
    {
        setError(
            QString("LAS %1.%2: No point records found in file").arg(header.versionMajor).arg(header.versionMinor));
        return false;
    }
...
std::vector<float> LasParser::readPointFormat0(QFile& file, const LasHeader& header)
{
    ...
    for (uint32_t i = 0; i < header.numberOfPointRecords; ++i)
    {
        ...
        {
            throw LasParseException(
                QString("Failed to skip point data for point %1 (expected %2 bytes)").arg(i).arg(bytesToSkip));
        }
...
std::vector<float> LasParser::readPointFormat1(QFile& file, const LasHeader& header)
{
    ...
    for (uint32_t i = 0; i < header.numberOfPointRecords; ++i)
    {
        ...
        // Standard PDRF 1: intensity, return info, classification, scan angle, user data, point source ID, GPS time =
        // 16 bytes Plus any vendor-specific extensions
        if (file.read(bytesToSkip).size() != bytesToSkip)
        {
            throw LasParseException(
                QString("Failed to skip point data for point %1 (expected %2 bytes)").arg(i).arg(bytesToSkip));
...
std::vector<float> LasParser::readPointFormat2(QFile& file, const LasHeader& header)
{
    ...
    for (uint32_t i = 0; i < header.numberOfPointRecords; ++i)
    {
        ...
        // Standard PDRF 2: intensity, return info, classification, scan angle, user data, point source ID, RGB = 14
        // bytes Plus any vendor-specific extensions
        if (file.read(bytesToSkip).size() != bytesToSkip)
        {
            throw LasParseException(
                QString("Failed to skip point data for point %1 (expected %2 bytes)").arg(i).arg(bytesToSkip));
...
std::vector<float> LasParser::readPointFormat3(QFile& file, const LasHeader& header)
{
    ...
    for (uint32_t i = 0; i < header.numberOfPointRecords; ++i)
    {
        ...
        // Standard PDRF 3: intensity, return info, classification, scan angle, user data, point source ID, GPS time,
        // RGB = 22 bytes Plus any vendor-specific extensions
        if (file.read(bytesToSkip).size() != bytesToSkip)
        {
            throw LasParseException(
                QString("Failed to skip point data for point %1 (expected %2 bytes)").arg(i).arg(bytesToSkip));
...
void LasParser::transformAndAddPoint(
    std::vector<float>& points, int32_t x, int32_t y, int32_t z, const LasHeader& header)
{
...
{
    return (major == SUPPORTED_VERSION_MAJOR && minor >= MIN_VERSION_MINOR && minor <= MAX_VERSION_MINOR);
}
...
uint16_t LasParser::getExpectedRecordLength(uint8_t pointDataFormat) const
{
    ...
    {
        case 0:
            return 20;  // XYZ, Intensity, Return info, Classification, Scan angle, User data, Point source ID
        case 1:
            return 28;  // PDRF 0 + GPS Time (8 bytes)
        case 2:
            return 26;  // PDRF 0 + RGB (6 bytes)
        case 3:
            return 34;  // PDRF 0 + GPS Time + RGB (14 bytes)
        default:
...
uint16_t LasParser::getExpectedHeaderSize(uint8_t versionMinor) const
{
    ...
    {
        case 2:
            return 227;  // LAS 1.2 minimum header size
        case 3:
            return 235;  // LAS 1.3 minimum header size (adds waveform data start)
        case 4:
            return 375;  // LAS 1.4 minimum header size (adds EVLR and extended point counts)
        default:
...
bool LasParser::validateRecordLength(const LasHeader& header) const
{
    ...
    {
        const_cast<LasParser*>(this)->setError(
            QString("LAS %1.%2 PDRF %3: Point data record length too short. Minimum %4, got %5")
                .arg(header.versionMajor)
                .arg(header.versionMinor)
                .arg(header.pointDataFormat)
                .arg(minimumLength)
    ...
    if (header.pointDataRecordLength > minimumLength)
    {
        qDebug()
            << QString(
                   "LAS %1.%2 PDRF %3: Extended record length detected. Standard: %4, Actual: %5 (+%6 vendor bytes)")
                   .arg(header.versionMajor)
                   .arg(header.versionMinor)
                   .arg(header.pointDataFormat)
                   .arg(minimumLength)
...
bool LasParser::validateScaleFactors(const LasHeader& header) const
{
    ...
    {
        const_cast<LasParser*>(this)->setError(
            QString("LAS %1.%2: Scale factor for X axis is zero, data may be invalid")
                .arg(header.versionMajor)
                .arg(header.versionMinor));
        return false;
    }
    ...
    {
        const_cast<LasParser*>(this)->setError(
            QString("LAS %1.%2: Scale factor for Y axis is zero, data may be invalid")
                .arg(header.versionMajor)
                .arg(header.versionMinor));
        return false;
    }
    ...
    {
        const_cast<LasParser*>(this)->setError(
            QString("LAS %1.%2: Scale factor for Z axis is zero, data may be invalid")
                .arg(header.versionMajor)
                .arg(header.versionMinor));
        return false;
    }
