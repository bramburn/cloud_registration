...
PerformanceProfiler::PerformanceProfiler(QObject* parent) : QObject(parent), m_enabled(true)
{
    m_globalTimer.start();
...
QString PerformanceProfiler::generateTextReport() const
{
    ...
    std::sort(sortedSections.begin(),
              sortedSections.end(),
              [](const ProfileSection& a, const ProfileSection& b) { return a.totalTime > b.totalTime; });
