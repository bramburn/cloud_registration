...
void OctreeNode::subdivide()
{
    ...
    children[0] = std::make_unique<OctreeNode>(AxisAlignedBoundingBox(min, center), depth + 1);
    children[1] = std::make_unique<OctreeNode>(
        AxisAlignedBoundingBox(QVector3D(center.x(), min.y(), min.z()), QVector3D(max.x(), center.y(), center.z())),
        depth + 1);
    children[2] = std::make_unique<OctreeNode>(
        AxisAlignedBoundingBox(QVector3D(min.x(), center.y(), min.z()), QVector3D(center.x(), max.y(), center.z())),
        depth + 1);
    children[3] = std::make_unique<OctreeNode>(
        AxisAlignedBoundingBox(QVector3D(center.x(), center.y(), min.z()), QVector3D(max.x(), max.y(), center.z())),
        depth + 1);
    children[4] = std::make_unique<OctreeNode>(
        AxisAlignedBoundingBox(QVector3D(min.x(), min.y(), center.z()), QVector3D(center.x(), center.y(), max.z())),
        depth + 1);
    children[5] = std::make_unique<OctreeNode>(
        AxisAlignedBoundingBox(QVector3D(center.x(), min.y(), center.z()), QVector3D(max.x(), center.y(), max.z())),
        depth + 1);
    children[6] = std::make_unique<OctreeNode>(
        AxisAlignedBoundingBox(QVector3D(min.x(), center.y(), center.z()), QVector3D(center.x(), max.y(), max.z())),
        depth + 1);
    children[7] = std::make_unique<OctreeNode>(AxisAlignedBoundingBox(center, max), depth + 1);
}
    ...
    else
    {
        ...
        if (distance < lodDistance2)
        {
            for (const auto& child : children)
            {
                if (child)
                {
                    child->collectVisiblePoints(frustumPlanes, cameraPos, lodDistance1, lodDistance2, visiblePoints);
                }
            }
        }
        else
                              ...
                              std::vector<PointFullData>& visiblePoints) const
{
    if (root)
    {
        root->collectVisiblePoints(frustumPlanes, cameraPos, lodDistance1, lodDistance2, visiblePoints);
    }
}
    ...
    else
    {
        ...
        for (const auto& child : children)
        {
            if (child)
            {
                ...
                m_representativePoints.insert(
                    m_representativePoints.end(), childRepPoints.begin(), childRepPoints.end());
            }
        }
...
void OctreeNode::collectVisiblePointsWithScreenSpaceError(const std::array<QVector4D, 6>& frustumPlanes,
                                                          ...
                                                          std::vector<PointFullData>& visiblePoints) const
{
    ...
    float screenSpaceError = ScreenSpaceErrorCalculator::calculateAABBScreenSpaceError(bounds, mvpMatrix, viewport);
    ...
    if (ScreenSpaceErrorCalculator::shouldStopRecursion(screenSpaceError, primaryThreshold))
    {
        ...
        visiblePoints.insert(visiblePoints.end(), representativePoints.begin(), representativePoints.end());
        return;
    }
        ...
        for (const auto& child : children)
        {
            if (child)
            {
                child->collectVisiblePointsWithScreenSpaceError(
                    frustumPlanes, mvpMatrix, viewport, primaryThreshold, cullThreshold, visiblePoints);
            }
        }
    }
}
...
void OctreeNode::calculateAggregateData() const
{
    if (isLeaf)
    {
        ...
        for (const auto& point : points)
        {
            ...
            if (point.hasColor())
            {
                colorSum += QVector3D(point.r.value() / 255.0f, point.g.value() / 255.0f, point.b.value() / 255.0f);
                colorCount++;
            }
        ...
        m_aggregateData.averageNormal = normalCount > 0 ? normalSum.normalized() : estimateNormalFromPoints();
...
void OctreeNode::collectRenderData(const std::array<QVector4D, 6>& frustumPlanes,
                                   const QMatrix4x4& mvpMatrix,
                                   const ViewportInfo& viewport,
                                   ...
                                   std::vector<AggregateNodeData>& splatData) const
{
    ...
    else
    {
        ...
        for (const auto& child : children)
        {
            if (child)
            {
                child->collectRenderData(
                    frustumPlanes, mvpMatrix, viewport, splatThreshold, splattingEnabled, individualPoints, splatData);
            }
        }
    }
}
