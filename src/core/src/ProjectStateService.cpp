...
public:
    ...
    void closeDatabase() {}
    bool updateScanFilePath(const QString&, const QString&)
    {
        return true;
    }
    bool deleteScan(const QString&)
    {
        return true;
    }
    ...
    bool saveAllClusters(const QList<ClusterInfo>&)
    {
        return true;
    }
    bool saveAllScans(const QList<ScanInfo>&)
    {
        return true;
    }
    ...
    bool insertCluster(const ClusterInfo&)
    {
        return true;
    }
    bool deleteCluster(const QString&)
    {
        return true;
    }
    bool updateClusterName(const QString&, const QString&)
    {
        return true;
    }
    bool updateScanCluster(const QString&, const QString&)
    {
        return true;
    }
    bool updateClusterLockState(const QString&, bool)
    {
        return true;
    }
    QStringList getScansInCluster(const QString&)
    {
        return QStringList();
    }
    struct Error
...
public:
    ...
    bool loadFromDatabase(SQLiteManager*)
    {
        return true;
    }
    ...
    QList<ScanInfo> getScansInClusterDetailed(const QString&) const
    {
        return QList<ScanInfo>();
    }
    ScanInfo getScanInfo(const QString&)
    {
        return ScanInfo();
    }
};
      ...
      m_validationTimer(std::make_unique<QTimer>())
{
    ...
    connect(m_validationTimer.get(), &QTimer::timeout, this, &ProjectStateService::onValidationTimerTimeout);
}
...
bool ProjectStateService::relinkScanFile(const QString& scanId, const QString& newFilePath)
{
    try
    {
        ...
        {
            setError("Failed to update scan file path in database", m_sqliteManager->lastError().text());
            return false;
        }
...
SaveResult ProjectStateService::saveProjectDatabaseTransactional()
{
    try
    {
        ...
        {
            m_sqliteManager->rollbackTransaction();
            setError("Referential integrity validation failed", "Database relationships are inconsistent");
            return SaveResult::DatabaseWriteFailed;
        }
...
void ProjectStateService::validateLinkedScanFile(const QString& scanId,
...
bool ProjectStateService::isValidProject(const QString& projectPath)
{
    ...
    return dir.exists() && QFileInfo::exists(getMetadataFilePath(projectPath)) &&
...
{
    QDir dir(path);
    return dir.exists() && QFileInfo::exists(ProjectStateService::getMetadataFilePath(path)) &&
