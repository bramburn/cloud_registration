...
PointCloud PointCloud::subsample(float ratio) const {
    ...
    if (ratio <= 0.0f) return PointCloud();
    
    PointCloud result;
    size_t targetSize = static_cast<size_t>(points.size() * ratio);
    
    if (targetSize == 0) return result;
...
std::unique_ptr<KDTree::Node> KDTree::buildTree(
    std::vector<std::pair<QVector3D, int>>& points, int depth) {
...
bool KDTree::findNearestNeighbor(const QVector3D& query, float maxDistance,
...
void KDTree::findNearest(const std::unique_ptr<Node>& node, const QVector3D& query,
                        QVector3D& bestPoint, float& bestDistance, int depth) const {
    if (!node) return;
...
ICPRegistration::ICPRegistration(QObject* parent)
    : QObject(parent), m_isCancelled(false), m_isRunning(false) {
}

QMatrix4x4 ICPRegistration::compute(const PointCloud& source, const PointCloud& target,
    ...
    qDebug() << "Starting ICP with" << source.size() << "source points and"
             << target.size() << "target points";

    // Subsample if requested
    PointCloud workingSource = (params.subsamplingRatio < 1.0f) ?
        source.subsample(params.subsamplingRatio) : source;
    PointCloud workingTarget = (params.subsamplingRatio < 1.0f) ?
        target.subsample(params.subsamplingRatio) : target;

    qDebug() << "Working with" << workingSource.size() << "source points and"
             << workingTarget.size() << "target points after subsampling";
    ...
    for (iteration = 0; iteration < params.maxIterations; ++iteration) {
        ...
        auto correspondences = findCorrespondences(transformedSource, workingTarget,
    ...
    if (!m_isCancelled) {
        ...
        auto finalCorrespondences = findCorrespondences(transformedSource, workingTarget,
                                                       params.maxCorrespondenceDistance);
    ...
    qDebug() << "ICP finished. Success:" << success << "Final RMS:" << finalError
...
std::vector<Correspondence> ICPRegistration::findCorrespondences(
    ...
    qDebug() << "Found" << correspondences.size() << "correspondences out of"
...
std::vector<Correspondence> ICPRegistration::removeOutliers(
    const std::vector<Correspondence>& correspondences, float threshold) {

    if (correspondences.empty()) return correspondences;
    ...
    if (distances.empty()) return correspondences;
    ...
    qDebug() << "Outlier removal: kept" << filtered.size() << "removed" << removedCount
