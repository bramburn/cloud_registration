...
PointToPlaneICP::PointToPlaneICP(QObject* parent) : ICPRegistration(parent) {}

QMatrix4x4 PointToPlaneICP::compute(const PointCloud& source,
                                    ...
                                    const ICPParams& params)
{
    ...
    qDebug() << "Starting Point-to-Plane ICP with" << source.size() << "source points and" << target.size()
             << "target points with normals";
...
PointToPlaneICP::findCorrespondences(const PointCloud& source, const PointCloud& target, float maxDistance)
{
    ...
    qDebug() << "Found" << correspondences.size() << "correspondences with normals out of" << source.size()
             << "source points";
...
QMatrix4x4 PointToPlaneICP::computeTransformation(const std::vector<Correspondence>& correspondences)
{
    ...
    {  // Need at least 6 correspondences for 6-DOF
